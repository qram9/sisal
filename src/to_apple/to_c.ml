...
let rec lower_graph env gr =
  let preds = ES.fold (fun ((sn, sp), (dn, dp), _) m ->
    PortMap.add (dn, dp) (sn, sp) m
  ) gr.eset PortMap.empty in
  let env = { env with preds } in
  let sorted_nids = topo_sort gr in
  List.fold_left (fun (acc_stmts, curr_env) nid ->
...
and lower_node env _gr nid node =
  ...
  | Simple (_, sym, _, _, _) ->
      ...
      | Some op ->
          ...
          let stmt = C.Decl (C.Basic "int32_t", v_name, Some (C.BinOp (op, e1, e2))) in
          let env = { env with var_map = PortMap.add (nid, 0) (C.Id v_name) env.var_map } in
          [stmt], env
      | None -> 
          ...
          | Some op ->
              ...
              let v_name = Printf.sprintf "v_%d" nid in
              let stmt = C.Decl (C.Basic "int32_t", v_name, Some (C.UnaryOp (op, e))) in
              let env = { env with var_map = PortMap.add (nid, 0) (C.Id v_name) env.var_map } in
              [stmt], env
          | None -> [], env
      end
      ...
      let params = match NM.find_opt 0 sub_gr.nmap with
        | Some (Boundary (ins, _, _, _)) ->
            List.map (fun (_, pid, name) ->
              let ty_id = ES.fold (fun ((sn, sp), (_dn, _dp), t) acc ->
