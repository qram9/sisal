  ...
  with _ -> 
    ...
    | Some (Boundary (ins, outs, _, _)) ->
        if nid = 0 && dir = `Out then
          let ty_id = List.find_map (fun (t, p, _) -> if p = pid then Some t else None) ins |> Option.value ~default:0 in
...
let rec lower_graph env gr gid =
  ...
  begin match NM.find_opt 0 gr.nmap with
    | Some (Boundary (ins, _, _, _)) ->
        List.iter (fun (ty_id, pid, name) ->
          let ty = get_port_type !env_ref gr 0 pid `Out in
          let local_v = var_name gid 0 pid `Out in
          let src_expr = try get_expr !env_ref gid 0 pid `Out with _ -> C.Id (sanitize name) in
          
          if src_expr <> C.Id local_v then
            agreement_body := C.Decl (ty, local_v, Some src_expr) :: !agreement_body;
          env_ref := { !env_ref with var_map = FullPortMap.add (gid, 0, pid, `Out) (C.Id local_v) !env_ref.var_map }
        ) ins
    | _ -> ()
  end;
  ...
  List.iter (fun pid ->
    ...
    env_ref := { !env_ref with var_map = FullPortMap.add (gid, 0, pid, `In) (C.Id local_res_v) !env_ref.var_map }
  ) all_b_outs;
  ...
  let computation_body = 
      ...
      if nid <> 0 then (
        let pids = match node with
          ...
          | Compound (_, _, _, _, sub_gr, _) ->
              ...
              ES.fold (fun (_, (dn, dp), _) acc -> if dn = 0 then dp :: acc else acc) sub_gr.eset []
          | _ -> [0]
        in
        List.iter (fun pid ->
          ...
          if not (Hashtbl.mem seen_decls v) then (
            ...
            env_ref := { !env_ref with var_map = FullPortMap.add (gid, nid, pid, `Out) (C.Id v) !env_ref.var_map }
          )
    ...
    let stmts = List.fold_left (fun acc nid ->
      ...
      match NM.find_opt nid gr.nmap with
      | Some node ->
          let node_preds = ES.filter (fun (_, (dn, dp), _) -> dn = nid) gr.eset in
          ES.iter (fun ((sn, sp), (_, dp), _) ->
            let src_expr = get_expr !env_ref gid sn sp `Out in
            env_ref := { !env_ref with var_map = FullPortMap.add (gid, nid, dp, `In) src_expr !env_ref.var_map }
          ) node_preds;
...
and lower_node env gr nid node =
  ...
  | Compound (cid, sy, ty, pr, loop_gr, assoc) ->
      ...
      if is_real_forall then
          ...
          let _, env_after_loop_header = lower_graph { env with var_map = var_map_child; parent_env = Some env } loop_gr sub_gid in
          
          let count_v = match gen_info with
            | Some (gen_nid, gen_gr) ->
                begin match NM.choose_opt (NM.filter (fun _ n -> match n with Simple(_, RANGEGEN, _, _, _) -> true | _ -> false) gen_gr.nmap) with
                | Some (rg_nid, _) -> get_expr env_after_loop_header sub_gid rg_nid 0 `Out
                | None -> 
                    match NM.choose_opt (NM.filter (fun _ n -> match n with Simple(_, ASIZE, _, _, _) -> true | _ -> false) gen_gr.nmap) with
                    | Some (size_nid, _) -> get_expr env_after_loop_header sub_gid size_nid 0 `Out
                    | None -> C.Id "0"
                end
            | None -> 
                match NM.choose_opt (NM.filter (fun _ n -> match n with Simple(_, ASIZE, _, _, _) -> true | _ -> false) loop_gr.nmap) with
                | Some (size_nid, _) -> get_expr env_after_loop_header sub_gid size_nid 0 `Out
                | None -> C.Id "0"
          in
          
          let res_v = get_expr env gid nid 0 `Out in
          let elem_ty = match get_elem_type env gr nid with Unknown_ty -> TM.find 8 env.tm | ty -> ty in
          ...
          | Some (body_id, body_gr) ->
              ...
              ES.iter (fun ((sn, sp), (dn, dp), _) ->
                if dn = body_id then
                  let expr = get_expr env_after_loop_header sub_gid sn sp `Out in
                  var_map_body := FullPortMap.add (body_gid, 0, dp, `Out) expr !var_map_body
              ) loop_gr.eset;
              ...
              begin match NM.find_opt 0 body_gr.nmap with
                | Some (Boundary (body_ins, _, _, _)) ->
                    List.iter (fun (_, pid, name) ->
                      let sn = sanitize name in
                      if sn = "i" || sn = "idx" || sn = "v_idx" then
                        var_map_body := FullPortMap.add (body_gid, 0, pid, `Out) (C.Id index_var) !var_map_body
                    ) body_ins
                | _ -> ()
              end;

              let body_stmts, env_after_body = lower_graph { env_after_loop_header with var_map = !var_map_body; parent_env = Some env_after_loop_header } body_gr body_gid in
              
              (* Find result from body subgraph (node 0 in-port 0) *)
              let body_res = ES.fold (fun ((sn, sp), (dn, dp), _) acc ->
                  if dn = 0 && dp = 0 then Some (get_expr env_after_body body_gid sn sp `Out) else acc
                ) body_gr.eset None |> Option.value ~default:(C.Id "0.0f") in
              
              let cast_ty = match c_type_of_if1_ty env.tm elem_ty with C.Basic s -> s | _ -> "float" in
              let cast_ptr = C.Cast (C.Pointer(C.Basic cast_ty, []), C.Member (res_v, "data")) in
              let update_stmt = C.Expr (C.BinOp (C.Assign, C.Index (cast_ptr, C.Id index_var), body_res)) in
              [C.Expr (C.BinOp (C.Assign, res_v, C.Call ("sisal_array_alloc_empty", [C.LitInt 1; C.LitInt tid; C.Cast (C.Basic "uint64_t", count_v)])));
               C.GCDApply (count_v, "dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)", (index_var, body_stmts @ [update_stmt]))]
          | None -> [C.Expr (C.BinOp (C.Assign, res_v, C.Call ("sisal_array_alloc_empty", [C.LitInt 1; C.LitInt tid; C.Cast (C.Basic "uint64_t", count_v)])))]
      else
          let sub_stmts, env_after_sub = lower_graph { env with var_map = var_map_child; parent_env = Some env } loop_gr sub_gid in
  ...
  | Simple (_, sym, pin, pout, pr) ->
      begin match match sym with
        ...
        | GREATER -> Some C.Gt | GREATER_EQUAL -> Some C.Ge | OR -> Some C.LogOr
        | AND -> Some C.LogAnd | _ -> None
      with
      | Some op ->
      ...
      | None -> 
          if sym = TYPECAST then
            ...
            [C.Expr (C.BinOp (C.Assign, v_res, C.Call ("sisal_array_create", [lb; C.LitInt tid; C.Cast (C.Basic "int32_t", size)])))]
...
let lower_procedure tm nid node =
  ...
  | Compound (_, INTERNAL, _, pr, sub_gr, _) ->
      ...
      let var_map = match NM.find_opt 0 sub_gr.nmap with
        | Some (Boundary (ins, _, _, _)) ->
            List.fold_left (fun m (ty_id, pid, name) ->
              FullPortMap.add (sub_gid, 0, pid, `Out) (C.Id (sanitize name)) m
            ) FullPortMap.empty ins
        | _ -> FullPortMap.empty in
      let env = { tm; var_map; preds = FullPortMap.empty; curr_gid = sub_gid; parent_env = None; force_gpu = is_gpu_func } in
      ...
      if ret_count > 1 then
          ...
          let final_body = body_stmts @ [C.Decl (C.Basic ("struct " ^ struct_name), res_var, None)] @ assigns @ [C.Return (Some (C.Id res_var))] in
          ...
          let ty = get_port_type env_after sub_gr 0 dp `In in
          Some ({ C.return_ty = ty; name = func_name; params; body = body_stmts @ [C.Return (Some e)]; extern_c = true }, None)
      else Some ({ C.return_ty = C.Void; name = func_name; params; body = body_stmts; extern_c = true }, None)
  | _ -> None
