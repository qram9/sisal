...
type env = {
  ...
  proc_param_map : C.expr FullPortMap.t;
      (* Procedure GID, NID, PID, Dir -> param name *)
  gid_name_map : string IntMap.t;
      (* GID to human-readable scope name from compound pragma *)
  procedures_info : Ir.If1.graph IntMap.t;
      (* Fast lookup for procedure subgraphs *)
  force_gpu : bool;
}
