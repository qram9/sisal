...
let get_port_type env gr nid pid dir =
  ...
  match ty_id with
  | Some tid -> (try TM.find tid env.tm |> c_type_of_if1_ty env.tm with _ -> C.Basic "int32_t")
...
let c_binop_of_sym = function
  | ADD -> Some C.Add | SUBTRACT -> Some C.Sub
  | TIMES -> Some C.Mul | FDIVIDE | IDIVIDE -> Some C.Div
  | EQUAL -> Some C.Eq | NOT_EQUAL -> Some C.Ne
  | LESSER -> Some C.Lt | LESSER_EQUAL -> Some C.Le
  | GREATER -> Some C.Gt | GREATER_EQUAL -> Some C.Ge
  | AND | BITAND -> Some C.BitAnd | OR | BITOR -> Some C.BitOr
  | _ -> None

let c_unaop_of_sym = function
  | NEGATE -> Some C.Negate | NOT -> Some C.LogNot | _ -> None
...
let rec lower_subgraph env gr gid =
  ...
  let b_ins, b_outs = match boundary with Boundary (i, o, _, _) -> i, o | _ -> [], [] in
  
  (* 1. Boundary Input Declarations and Bindings *)
  let env_ref = ref { env with curr_gid = gid } in
  let b_in_decls = List.map (fun (ty_id, pid, name) ->
    let s_name = sanitize name in
    let ty = try TM.find ty_id env.tm |> c_type_of_if1_ty env.tm with _ -> C.Basic "void*" in
    let v_name = var_name gid 0 pid in
    (* Bind from parent if possible, otherwise use name *)
    let init = try Some (get_expr env gid 0 pid) with _ -> Some (C.Id s_name) in
    env_ref := { !env_ref with var_map = PortMap.add (gid, 0, pid) (C.Id v_name) !env_ref.var_map };
    C.Decl (ty, v_name, init)
  ) b_ins in

  (* 2. Boundary Output Declarations (pre-declare) *)
  let b_out_decls = List.map (fun (ty_id, pid) ->
    let ty = try TM.find ty_id env.tm |> c_type_of_if1_ty env.tm with _ -> C.Basic "void*" in
    let v_name = var_name gid 0 pid in
    env_ref := { !env_ref with var_map = PortMap.add (gid, 0, pid) (C.Id v_name) !env_ref.var_map };
  ...
  NM.iter (fun nid node ->
    if nid <> 0 then (
      ...
      List.iter (fun pid ->
        ...
        env_ref := { !env_ref with var_map = PortMap.add (gid, nid, pid) (C.Id v) !env_ref.var_map }
      ) pids
    )
  ) gr.nmap;
  ...
  let computation_stmts = List.fold_left (fun acc nid ->
  ...
  (* 5. Propagation to Boundary Outputs *)
  ...
  let inner_block = C.Compound ((List.rev !inner_decls) @ computation_stmts @ propagation) in
...
and lower_node env gr nid node =
  ...
  match node with
  | Simple (_, sym, _, _, _) ->
      begin match c_binop_of_sym sym with
      | Some op ->
      ...
      | None -> 
          ...
          | None ->
              if sym = DV_ELEMENT || sym = AELEMENT then
                ...
                let cast_ptr = C.Cast (C.Pointer(c_elem_ty, []), C.Member (arr, "data")) in
                let idx_expr = C.BinOp (C.Sub, idx, C.Member (arr, "lower_bound")) in
                [C.Expr (C.BinOp (C.Assign, v_res, C.Index (cast_ptr, idx_expr)))], env
              else if sym = ASIZE || sym = DV_DIMENSION then
                let arr = get_expr env gid nid 0 in
                let v_res = get_expr env gid nid 0 in
                [C.Expr (C.BinOp (C.Assign, v_res, C.Member (arr, "size")))], env
              else if sym = DV_CREATE then
                let lb = get_expr env gid nid 0 in
                let size = get_expr env gid nid 1 in
                let v_res = get_expr env gid nid 0 in
                (* Simplified DV_CREATE for 1D *)
                let tid = match get_elem_type env.tm gr nid with
                  | Basic REAL -> 0 | Basic DOUBLE -> 1 | Basic INTEGRAL -> 2 | Basic BOOLEAN -> 3 | _ -> 0 in
                [C.Expr (C.BinOp (C.Assign, v_res, C.Call ("sisal_array_create", [lb; C.LitInt tid; C.Cast (C.Basic "int32_t", size)])))], env
                ...
                let v_res = get_expr env gid nid 0 in
                [C.Expr (C.BinOp (C.Assign, v_res, C.Cond (cond, v_then, v_else)))], env
              else [], env
      end
  ...
  | Compound (_, INTERNAL, _, pr, sub_gr, _) ->
      ...
      let sub_block, env_after = lower_subgraph { env with parent_env = Some env } sub_gr sub_gid in
      (* Assign results back to this node's outputs *)
      let assignments = ES.fold (fun ((sn, sp), (dn, dp), _) acc ->
        if dn = 0 then
          let src = get_expr env_after sub_gid sn sp in
...
let lower_procedure tm nid node =
  ...
  | Compound (_, INTERNAL, _, pr, sub_gr, _) ->
      ...
      let params = match NM.find_opt 0 sub_gr.nmap with
        | Some (Boundary (ins, _, _, _)) ->
            List.map (fun (ty_id, pid, name) ->
              let ty = try TM.find ty_id tm |> c_type_of_if1_ty tm with _ -> C.Basic "void*" in
              (ty, sanitize name)
            ) ins
        | _ -> [] in
      
      let env = { tm; var_map = PortMap.empty; preds = PortMap.empty; curr_gid = sub_gid; parent_env = None } in
      ...
      let body = match ret_exprs with
        | (_, e) :: _ -> body @ [C.Return (Some e)]
        | [] -> body
      in
      
      let ret_ty_id = ES.fold (fun ((_sn, _sp), (dn, dp), t) acc ->
        if dn = 0 && dp = 0 then Some t else acc
      ) sub_gr.eset None |> Option.value ~default:0 in
      let ret_ty = try TM.find ret_ty_id tm |> c_type_of_if1_ty tm with _ -> C.Void in
      
      Some { C.return_ty = ret_ty; name = func_name; params; body; extern_c = true }
  | _ -> None

let translate (gr : graph) : C.translation_unit =
  let _, tm, _ = gr.typemap in
  next_gid := 0;
  let procedures = NM.fold (fun id node acc ->
