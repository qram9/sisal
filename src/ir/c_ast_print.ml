...
let rec string_of_c_type = function
  ...
  | Pointer (ty, quals) ->
      ...
      Printf.sprintf "%s *%s" (string_of_c_type ty)
        (if q_str = "" then "" else q_str ^ " ")
  | Array (ty, Some size) -> Printf.sprintf "%s[%d]" (string_of_c_type ty) size
  | Array (ty, None) -> Printf.sprintf "%s[]" (string_of_c_type ty)
  | Vector (ty, size) ->
      Printf.sprintf "%s __attribute__((vector_size(%d)))" (string_of_c_type ty)
        size
  | Struct (name, fields) ->
...
let rec string_of_expr = function
  ...
  | UnaryOp (op, e) -> begin
      match op with
      | PostInc | PostDec ->
          Printf.sprintf "(%s%s)" (string_of_expr e) (string_of_unary_op op)
  ...
  | Cast (ty, e) ->
      Printf.sprintf "((%s)%s)" (string_of_c_type ty) (string_of_expr e)
  | Cond (c, t, f) ->
...
let rec string_of_stmt indent =
  ...
  | For (init, cond, step, body) ->
      ...
      Printf.sprintf "%sfor (%s; %s; %s) {\n%s\n%s}" pad init_s
        (string_of_expr cond) (string_of_expr step)
        (String.concat "\n" (List.map (string_of_stmt (indent + 1)) body))
        pad
  | While (cond, body) ->
      Printf.sprintf "%swhile (%s) {\n%s\n%s}" pad (string_of_expr cond)
        (String.concat "\n" (List.map (string_of_stmt (indent + 1)) body))
        pad
  | DoWhile (body, cond) ->
      Printf.sprintf "%sdo {\n%s\n%s} while (%s);" pad
  ...
  | If (cond, t, f) ->
      let then_s =
        String.concat "\n" (List.map (string_of_stmt (indent + 1)) t)
      in
      let else_s =
        if f = [] then ""
        else
          Printf.sprintf "\n%selse {\n%s\n%s}" pad
            (String.concat "\n" (List.map (string_of_stmt (indent + 1)) f))
            pad
      in
      Printf.sprintf "%sif (%s) {\n%s\n%s}%s" pad (string_of_expr cond) then_s
        pad else_s
  ...
  | MetalKernel k ->
      let params_s =
        ...
        |> List.map (fun (ty, name, idx) ->
            Printf.sprintf "device %s %s [[buffer(%d)]]" (string_of_c_type ty)
              name idx)
        |> String.concat ", "
      in
      Printf.sprintf "%skernel void %s(%s) {\n%s\n%s}" pad k.name params_s
