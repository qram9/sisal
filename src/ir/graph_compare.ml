(** Structural comparison of IF1 graphs.
    ...
    literal value mismatches, type mismatches on edges). Compound subgraphs are
    compared recursively.

    2. {b Tree/DAG match}: strips boundary edges (those incident to node 0),
    then represents each output as a [cdag_node] tree. Two graphs are tree-equal
    when every corresponding output tree is equal. Fan-out (value reuse) is
    handled by memoising already-visited (node, port) pairs so each shared
    sub-computation is only compared once. *)
...
let node_label_str = function
  | Simple (_, sym, _, _, _) ->
      Printf.sprintf "Simple(%s)" (string_of_node_sym sym)
  | Compound (_, sym, _, _, _, _) ->
      Printf.sprintf "Compound(%s)" (string_of_node_sym sym)
  | Literal (_, code, v, _) ->
      Printf.sprintf "Literal(%s:%s)" (string_of_if1_basic_ty code) v
  | Boundary _ -> "Boundary"
...
let internal_edges gr = ES.filter (fun e -> not (is_boundary_edge e)) gr.eset
...
type diff =
  ...
  | Edge_extra_left of (int * int) * (int * int) * int
      (** (src,p),(dst,p),ty in g1 not g2 *)
  | Edge_extra_right of (int * int) * (int * int) * int
      (** (src,p),(dst,p),ty in g2 not g1 *)
  | Edge_type of (int * int) * (int * int) * int * int
      (** same endpoints, different types *)
  | Sub_mismatch of int * diff list  (** compound node, inner diffs *)

(** Human-readable diff entry. *)
let rec string_of_diff = function
  | Node_extra_left (id, tag) ->
      Printf.sprintf "  +node %d (%s) only in left" id tag
  | Node_extra_right (id, tag) ->
      Printf.sprintf "  +node %d (%s) only in right" id tag
  | Node_opcode (id, a, b) -> Printf.sprintf "  node %d opcode %s ≠ %s" id a b
  | Node_literal_val (id, a, b) ->
      Printf.sprintf "  node %d literal value %s ≠ %s" id a b
  | Node_literal_code (id, a, b) ->
      Printf.sprintf "  node %d literal type %s ≠ %s" id a b
...
type match_result = Graphs_equal | Graphs_differ of diff list
...
let rec compare_one_node id n1 n2 : diff list =
  ...
  | Simple (_, sym1, pin1, pout1, _), Simple (_, sym2, pin2, pout2, _) ->
      let d1 =
        if sym1 = sym2 then []
          ...
          [ Node_opcode (id, string_of_node_sym sym1, string_of_node_sym sym2) ]
      in
      let d2 =
        if Array.length pin1 = Array.length pin2 then []
        else
          [
            Node_opcode
              ( id,
                Printf.sprintf "%s(in=%d)" (string_of_node_sym sym1)
                  (Array.length pin1),
                Printf.sprintf "%s(in=%d)" (string_of_node_sym sym2)
                  (Array.length pin2) );
          ]
      in
      let d3 =
        if Array.length pout1 = Array.length pout2 then []
        else
          [
            Node_opcode
              ( id,
                Printf.sprintf "%s(out=%d)" (string_of_node_sym sym1)
                  (Array.length pout1),
                Printf.sprintf "%s(out=%d)" (string_of_node_sym sym2)
                  (Array.length pout2) );
          ]
      in
      d1 @ d2 @ d3
  | Literal (_, code1, v1, _), Literal (_, code2, v2, _) ->
      let d1 =
        if code1 = code2 then []
          ...
          [
            Node_literal_code
              (id, string_of_if1_basic_ty code1, string_of_if1_basic_ty code2);
          ]
      in
      let d2 =
        if String.equal v1 v2 then [] else [ Node_literal_val (id, v1, v2) ]
      in
      d1 @ d2
  | Compound (_, sym1, lbl1, _, g1, _), Compound (_, sym2, lbl2, _, g2, _) ->
      ...
      let d3 =
        match sub_diffs with [] -> [] | ds -> [ Sub_mismatch (id, ds) ]
      in
      d1 @ d2 @ d3
  | _ ->
...
type cdag_node =
  | Cdag_input of int
      (** Boundary input: the integer is the port on node 0. Represents a
          function parameter or loop-carried variable. *)
  | Cdag_literal of basic_code * string  (** A constant value. *)
  | Cdag_simple of node_sym * cdag_node array
      (** A Simple node with opcode [sym] and one child per input port (indexed
          0 … n-1, in port order). Ports with no incoming edge get
          [Cdag_input (-1)] as a placeholder. *)
  | Cdag_compound of node_sym * int * cdag_node array * graph
      (** A Compound node: opcode, label, children feeding the sub-boundary, and
          the (raw) subgraph for deeper inspection. *)
  | Cdag_unknown of int
...
let rec cdag_equal a b =
  ...
  | Cdag_literal (c1, v1), Cdag_literal (c2, v2) ->
      c1 = c2 && String.equal v1 v2
...
let build_cdag gr =
    ...
    ES.fold
      (fun ((sn, sp), (dn, dp), _) acc ->
        ...
        if List.mem_assoc k acc then acc
          (* keep first; valid IF1 is single-producer *)
        else (k, (sn, sp)) :: acc)
      int_edges []
  in
  ...
  let rec trace ?(seen = []) node_id out_port =
    ...
    | Some cached -> cached
    | None ->
        if List.mem node_id seen then Cdag_unknown node_id (* cycle guard *)
        else begin
          ...
          let result =
            ...
            | Some (Boundary _) ->
                ...
                Cdag_input out_port
            | Some (Literal (_, code, v, _)) -> Cdag_literal (code, v)
            | Some (Simple (_, sym, pin, _, _)) ->
                ...
                let children =
                  Array.init n_in (fun p ->
                      ...
                      | None ->
                          Cdag_input (-1)
                          (* dangling — boundary or unconnected *)
                      | Some (sn, sp) ->
            ...
            | Some (Compound (_, sym, lbl, _, sub, _)) ->
                ...
                let sub_in_count =
                  ...
                  | Some (Boundary (_, out_ports, _, _)) ->
                      List.length out_ports
                  | _ -> 0
                in
                let children =
                  Array.init sub_in_count (fun p ->
...
(** Extract the list of output trees from a graph: one [cdag_node] per output
    port of the boundary node, in port order. *)
...
let dag_equal g1 g2 : bool =
  ...
  List.length t1 = List.length t2 && List.for_all2 cdag_equal t1 t2

(** Structural comparison that uses the DAG representation. Gives a richer
    result than [dag_equal]. *)
...
let rec string_of_cdag ?(indent = 0) node =
  ...
  | Cdag_simple (sym, children) ->
      ...
      if Array.length children = 0 then header
        ...
        let body =
          ...
          |> List.mapi (fun i ch ->
              Printf.sprintf "%s  [%d]:\n%s" pad i
                (string_of_cdag ~indent:(indent + 4) ch))
          |> String.concat "\n"
        in
        header ^ "\n" ^ body
  | Cdag_compound (sym, lbl, children, _) ->
      ...
      if Array.length children = 0 then header
        ...
        in
        header ^ "\n" ^ body
  | Cdag_unknown id -> Printf.sprintf "%sUNKNOWN(%d)" pad id

let string_of_cdag_list cdags =
  List.mapi
    (fun i t -> Printf.sprintf "output[%d]:\n%s" i (string_of_cdag ~indent:2 t))
    cdags
  |> String.concat "\n"
