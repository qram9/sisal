...
let test_if_statement () =
  ...
  let then_body =
    [ Decl (Basic "int", "y", None); Expr (BinOp (AssignAdd, Id "a", Id "b")) ]
  in
...
let test_for_loop () =
  ...
  let body =
    [ Decl (Basic "int", "y", None); Expr (BinOp (Assign, Id "y", Id "i")) ]
  in
  ...
  let proc =
    {
      ...
      body = [ Return (Some (BinOp (Add, Id "x", Cast (Basic "int", Id "y")))) ];
      extern_c = false;
    }
  in
