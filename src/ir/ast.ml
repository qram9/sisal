...
and simple_exp =
  ...
  | Permute of
      simple_exp * simple_exp list (* PERMUTE(A, d0, d1, ..) reorder dims *)
  | Reduce of reduction_op * simple_exp (* SUM(A), PRODUCT(A), etc. *)
  | Reduce_range of
      reduction_op * simple_exp * simple_exp * simple_exp (* SUM(A,lo,hi) *)
  ...
  | Outerproduct_exp of
      simple_exp * simple_exp * simple_exp (* OUTERPRODUCT(f, A, B) *)
  | Grade_up_exp of
      simple_exp (* GRADE_UP(A) / ARGSORT(A) — ascending sort indices *)
  ...
  (* Search / index *)
  | Argmax_exp of
      simple_exp * simple_exp option (* ARGMAX(A) or ARGMAX(A,axis) *)
  | Argmin_exp of
      simple_exp * simple_exp option (* ARGMIN(A) or ARGMIN(A,axis) *)
  | Nonzero_exp of simple_exp (* NONZERO(A) → indices of nonzero *)
  | Where_exp of
      simple_exp
      * simple_exp
      * simple_exp (* WHERE(cond,A,B) element-wise select *)
  (* Statistical reductions — optional axis as 2nd positional arg *)
  | Reduce_axis of
      reduction_op
      * simple_exp
      * simple_exp (* SUM/PRODUCT/LEAST/GREATEST(A,k) *)
  | Mean_exp of simple_exp * simple_exp option
  | Dv_offset_at of
      simple_exp * simple_exp * simple_exp (* array, index, common_shape *)
  ...
  (* Shape manipulation *)
  | Concat_exp of
      simple_exp * simple_exp (* CONCAT(A,B) / CATENATE(A,B) — APL dyadic , *)
  ...
  (* Stencil / filter *)
  | Stencil_exp of
      simple_exp * simple_exp * simple_exp list (* STENCIL(f,A,d0,d1,..) *)
  | Pad_exp of
      simple_exp
      * simple_exp
      * simple_exp
      * simple_exp option (* PAD(A,lo,hi[,fill]) *)
  (* Linear algebra *)
  | Innerproduct_exp of
...
and str_simple_exp ?(offset = 0) ?(preceed_space = 1) = function
  ...
  | Reshape (arr, dims) ->
      let dims_str =
        String.concat ", " (List.map (fun d -> str_simple_exp d) dims)
      in
  ...
  | Reduce (op, e) -> " " ^ str_reduction op ^ "(" ^ str_simple_exp e ^ ")"
  | Reduce_range (op, e, lo, hi) ->
      " " ^ str_reduction op ^ "(" ^ str_simple_exp e ^ ", " ^ str_simple_exp lo
      ^ ", " ^ str_simple_exp hi ^ ")"
  | Each_exp (f, a) ->
      " EACH(" ^ str_simple_exp f ^ ", " ^ str_simple_exp a ^ ")"
  | Foldl_exp (f, init, a) ->
      " FOLDL(" ^ str_simple_exp f ^ ", " ^ str_simple_exp init ^ ", "
      ^ str_simple_exp a ^ ")"
  | Foldr_exp (f, init, a) ->
      " FOLDR(" ^ str_simple_exp f ^ ", " ^ str_simple_exp init ^ ", "
      ^ str_simple_exp a ^ ")"
  | Scan_exp (f, a) ->
  ...
  | Outerproduct_exp (f, a, b) ->
      " OUTERPRODUCT(" ^ str_simple_exp f ^ ", " ^ str_simple_exp a ^ ", "
      ^ str_simple_exp b ^ ")"
  | Grade_up_exp a -> " GRADE_UP(" ^ str_simple_exp a ^ ")"
  | Grade_down_exp a -> " GRADE_DOWN(" ^ str_simple_exp a ^ ")"
  | Sort_exp a -> " SORT(" ^ str_simple_exp a ^ ")"
  | Broadcast_exp (s, a) ->
      " BROADCAST(" ^ str_simple_exp s ^ ", " ^ str_simple_exp a ^ ")"
  | Argmax_exp (a, None) -> " ARGMAX(" ^ str_simple_exp a ^ ")"
  | Argmax_exp (a, Some k) ->
      " ARGMAX(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Argmin_exp (a, None) -> " ARGMIN(" ^ str_simple_exp a ^ ")"
  | Argmin_exp (a, Some k) ->
      " ARGMIN(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Nonzero_exp a -> " NONZERO(" ^ str_simple_exp a ^ ")"
  | Where_exp (c, a, b) ->
      " WHERE(" ^ str_simple_exp c ^ ", " ^ str_simple_exp a ^ ", "
      ^ str_simple_exp b ^ ")"
  | Reduce_axis (op, a, k) ->
  ...
  | Mean_exp (a, Some k) ->
      " MEAN(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Dv_offset_at (a, i, s) ->
      " DV_OFFSET_AT(" ^ str_simple_exp a ^ ", " ^ str_simple_exp i ^ ", "
      ^ str_simple_exp s ^ ")"
  | Dv_load_linear (a, o) ->
      " DV_LOAD_LINEAR(" ^ str_simple_exp a ^ ", " ^ str_simple_exp o ^ ")"
  | Dv_num_rank a -> " DV_NUM_RANK(" ^ str_simple_exp a ^ ")"
  | Dv_dimension (a, k) ->
      " DV_DIMENSION(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Variance_exp (a, None) -> " VARIANCE(" ^ str_simple_exp a ^ ")"
  | Variance_exp (a, Some k) ->
      " VARIANCE(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Stddev_exp (a, None) -> " STDDEV(" ^ str_simple_exp a ^ ")"
  | Stddev_exp (a, Some k) ->
      " STDDEV(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Any_exp (a, None) -> " ANY(" ^ str_simple_exp a ^ ")"
  | Any_exp (a, Some k) ->
      " ANY(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | All_exp (a, None) -> " ALL(" ^ str_simple_exp a ^ ")"
  | All_exp (a, Some k) ->
      " ALL(" ^ str_simple_exp a ^ ", " ^ str_simple_exp k ^ ")"
  | Norm_exp (a, p) ->
      " NORM(" ^ str_simple_exp a ^ ", " ^ str_simple_exp p ^ ")"
  | Cumsum_exp a -> " CUMSUM(" ^ str_simple_exp a ^ ")"
  | Cumprod_exp a -> " CUMPROD(" ^ str_simple_exp a ^ ")"
  | Concat_exp (a, b) ->
      " CONCAT(" ^ str_simple_exp a ^ ", " ^ str_simple_exp b ^ ")"
  | Tile_exp (a, n) ->
      " TILE(" ^ str_simple_exp a ^ ", " ^ str_simple_exp n ^ ")"
