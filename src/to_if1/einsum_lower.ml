...
type subscript_info = {
  ...
  has_ellipsis : bool;  (** True when "..." appears anywhere in the subscript. *)
}
...
(** Parse an einsum subscript string. Raises [Failure] on malformed input. *)
let parse_einsum_subscript (s : string) : subscript_info =
  let has_ellipsis = Re.(execp (compile (str "..."))) s in
...
(** For each unique index label, find the FIRST operand + dimension-index where
...
let free_labels (info : subscript_info) : char list = info.output_spec
