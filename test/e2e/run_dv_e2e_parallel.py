...
def _load_parts_index():
    """MACRO -> part file.  Generated by split_harness.py; see
    docs/e2e_harness_split_plan.md.  Falls back to the unsplit harness."""
    idx = os.path.join(PARTS, "parts.index")
    if not os.path.exists(idx):
        return {}
    out = {}
    for line in open(idx):
        if line.split():
            macro, part = line.split()
            out[macro] = os.path.join(PARTS, part)
    return out

PARTS_INDEX = _load_parts_index()
HARNESS = os.path.join(REPO, "test/e2e/dv_run_all.cpp")

def harness_for(macro):
    return PARTS_INDEX.get(macro, HARNESS)
NUMPY_VERIFY = os.path.join(REPO, "test/e2e/numpy_verify.cpp")
E2E_SH = os.path.join(REPO, "test/e2e/run_dv_e2e.sh")

CXX_BASE = [
    "clang++", "-std=c++23", "-O3", "-ffast-math", f"-I{RUNTIME}", f"-I{os.path.join(REPO, 'test/e2e')}",
...
def run_single_test(args):
    ...
    cmd_build = CXX_BASE + [
        f"-DTEST_{macro}", harness_for(macro), NUMPY_VERIFY, cpp_out, "-o", bin_out
    ]
