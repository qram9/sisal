...
// Groups: HOF_PASSING_DV, HOF_LIST_DV, LETREC_SCOPE_DV, ADA_DV, BUBBLE_DV, SCAT_DV, PINSERTDATA_DV, SBATCHER_DV, PBATCHER_DV, NEWQUEENS_DV, GEN_EXTENT_DV, GAUSSJ1_DV, HELLO_DV, HELPER_FUNCS_DV, QUICKSORT_WHEN_DV, CHOOSE_DV, FUNCARRAY_DV, COMMON_DV, FROMC_DV, LTSTEP_DV, INITFFT_DV, EINSUM_DV
#include "dv_harness.h"

#ifdef TEST_EINSUM_DV
struct EINSUM_DV_results {
  int32_t dot_res;
  int32_t outer_sz;
  int32_t tr_res;
  int32_t trans_sz;
...
extern "C" struct EINSUM_DV_results func_MAIN(sisal_array_t u, sisal_array_t v, sisal_array_t A_sq, sisal_array_t Mat);

static void test_einsum_dv(void) {
  printf("\n=== Group: einsum_dv (EINSUM tensor contraction vs C reference) ===\n");

  int32_t u_data[] = {1, 2, 3};
  int32_t v_data[] = {4, 5, 6};
  int32_t a_data[] = {1, 2, 3, 4}; // 2x2
  float m_data[] = {1.0f, 2.0f, 3.0f, 4.0f}; // 2x2

  sisal_array_t u = make_int_arr(u_data, 3);
  sisal_array_t v = make_int_arr(v_data, 3);
  sisal_array_t A_sq = make_int_2d(a_data, 2, 2);
  sisal_array_t Mat = make_float_2d(m_data, 2, 2);

  struct EINSUM_DV_results res = func_MAIN(u, v, A_sq, Mat);

  // Ground truth C reference calculations:
  // 1. dot_res = EINSUM("i,i->", u, v) = 1*4 + 2*5 + 3*6 = 32
  // 2. outer_sz = array_size(EINSUM("i,j->ij", u, v)) = 3 (leading dim of 3x3 matrix)
  // 3. tr_res = EINSUM("ii->", A_sq) = 1 + 4 = 5
  // 4. trans_sz = array_size(EINSUM("ij->ji", Mat)) = 2 (leading dim of 2x2 matrix)
  // 5. triple_res = EINSUM("ijk,ijk->", Mat, Mat) = 1^2 + 2^2 + 3^2 + 4^2 = 30.0f
  int32_t ref_dot = 1*4 + 2*5 + 3*6;                             // 32
  int32_t ref_outer_sz = 3;                                      // 3 (leading dim)
  int32_t ref_trace = 1 + 4;                                     // 5
  int32_t ref_trans_sz = 2;                                      // 2 (leading dim)
  float ref_triple = 1.0f*1.0f + 2.0f*2.0f + 3.0f*3.0f + 4.0f*4.0f; // 30.0f

  check("EINSUM dot product ('i,i->')", res.dot_res == ref_dot);
  check("EINSUM outer product leading size ('i,j->ij')", res.outer_sz == ref_outer_sz);
  check("EINSUM matrix trace ('ii->')", res.tr_res == ref_trace);
  check("EINSUM matrix transpose leading size ('ij->ji')", res.trans_sz == ref_trans_sz);
  check("EINSUM triple contraction ('ijk,ijk->')", fabsf(res.triple_res - ref_triple) < 1e-4f);
}
#endif
