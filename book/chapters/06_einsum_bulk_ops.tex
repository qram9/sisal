...
Rather than introducing a separate keyword, \texttt{EINSUM} itself is extended to support on-the-fly higher-order function (HOF) fusion for pre-processing loads and post-processing stores directly in the subscript string using bound function names:
...
function FusedSwiGLUFFN( X, W1, W2, W3 : array_dv[double] returns array_dv[double] )
  let
    SwiGLU := function(gate_val, up_val : double returns double)
      (gate_val / (1.0d0 + exp(-gate_val))) * up_val
    end function;
...
This unifies (1) tensor contraction math, (2) L1/L2 cache tiling, (3) loop interchange, (4) pre-processing load HOFs, and (5) post-processing store HOFs \textbf{all directly inside \texttt{EINSUM}} with zero extra language keywords!
