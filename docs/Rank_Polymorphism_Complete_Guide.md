...
## The Invariant That Dissolves the Ambiguity (the exit route)

The "ambiguity" in finding #1 only exists because the **last** index is special-cased to
mean "element". Drop that special case and adopt one invariant, and the whole problem
goes away:

> **A subscript always rank-reduces. `DV_LOAD_LINEAR` is the *sole* way to produce a
> scalar value.**

So `A[i]` is *always* the slice (rank decremented by one), with **no** element-vs-slice
decision at the subscript at all:

```
A : rank 3 [m,n,k]   →   A[i]   : rank 2 [n,k]
                          A[i,j] : rank 1 [k]
                          A[i,j,l] : rank 0 array_dv  (a 1-element view; NOT a raw scalar)
```

A raw value is obtained only by an explicit **dead-reckon**:

```
DV_LOAD_LINEAR(view, 0)  =  view.data[0]   (raw offset, no lower_bound)
```

Because `DV_RANK_REDUCE` already advanced `view.data` to the selected slice, loading
linear-0 yields the element the view points at automatically (e.g. `A[i]` on `[m,n]` is a
rank-1 view at offset `i*n`; `DV_LOAD_LINEAR(·,0)` gives `A[i,0]`).

**Why this is the exit route:** `sum(a[i])`, `a[i]` on a high-rank array, partial
subscripts — all become trivially well-formed. The subscript needs *no* type inference;
the only place context matters is deciding **where to insert a `DV_LOAD_LINEAR`** (a scalar
boundary), and the default (none) is "stay an array". This replaces the `expected_ty`
element-vs-slice inference with a much narrower question.

**The trade:** `a[i]` on a rank-1 array → rank-0 `array_dv`, so every scalar use
(`a[i]+1`, a `double` argument) needs a `DV_LOAD_LINEAR` (or the compiler-inserted one),
else it pays the rank-0 wrap / heap cost (the Gauss-Jordan inner-loop concern above). The
current `DV_ELEMENT`-flat-load shortcut is the opposite bet — cheap scalars, but it
silently mis-ranks partial subscripts.

### Where the code stands vs. this invariant (Jun 2026)

| op | name(s) | status |
|----|---------|--------|
| rank-reduce node | code: `DV_RANK_REDUCE` (opcode 130); doc: `DV_RANK_DEC` | **defined + runtime (`sisal_dv_rank_reduce`) + lowered (`apple_lower.ml:471`), but NEVER generated by `to_if1`** — orphan node |
| subscript | `DV_ELEMENT` (opcode 79) | born at the one site that matters: `to_if1.ml:5497` (`Array_ref`/`lower_indices`); also name-dispatched builtins `DV_ELEMENT(A,i)`/`(A,i,j)` (5348/5364) |
| scalar load | `DV_LOAD_LINEAR` (opcode 125) | **user-given only** — name-dispatched builtin (`to_if1.ml:5332`); compiler never auto-emits it. No grammar keyword. |

**The deviation to fix:** `lower_indices` (`to_if1.ml:5497`) types the **last** index as
the *element* type (`rest = [] → ij`), and `apple_lower.ml:445` flat-loads that scalar.
That "last index → element" rule is the *only* thing standing between the code and this
invariant. To honor it: make every `DV_ELEMENT` output `array_dv` (it would then route to
`DV_RANK_REDUCE` semantics, un-orphaning that node), and have scalars come **only** from
`DV_LOAD_LINEAR` — user-written now, auto-inserted at scalar boundaries by the deferred
context pass later. The live backend is `apple_lower.ml` (via `To_apple.translate`);
`to_c_backend.ml` and `apple_lower_legacy.ml` carry stale `DV_ELEMENT` logic and are dead.

---

## Alternative convention: load-by-default + explicit slice (the Fortran spelling)

A second option, *opposite* to the always-rank-reduce invariant above: make the bracket
**always a scalar load**, and require **explicit syntax** for a slice. Written down here
to evaluate whether it matches Fortran/APL/NumPy.

```
A[i,j]   full subscripts        → scalar LOAD
A[i]     full subscript (1-D)   → scalar LOAD   (partial bracket on a higher-rank array
                                                 is NOT an implicit rank-reduce)
row/slice → EXPLICIT:  A[i, ..]            (the Sisal-2.0 '..' axis placeholder)
                       SLICE(A, lo, hi)    (existing intrinsic → DV_SLICE, zero-copy view)
```

**What already exists for slices** (so this isn't from scratch):
- `DV_SLICE(A, starts, sizes) → zero-copy view` — `if1.ml:155` (opcode 83), runtime
  `sisal_array_slice`, generated from the `SLICE(arr, lo, hi)` builtin
  (`to_if1.ml:4486`) / `Slice_exp` AST (`to_if1.ml:8154`).
- `..` (DOTDOT) is the Sisal-2.0 triplet/placeholder (`for i in [1..4]`,
  `at [I,..,..]` scatters planes — `..` = "all of this axis"). It is **the natural
  Sisal "whole axis" marker** — Fortran's `:`.
- **`A[i, :]` is NOT available**: inside brackets `:` is already the functional
  REPLACE form (`A[i: v]` → `array_replace`, `parse.mly:simple_expr_pair`). Use `..`,
  not `:`.

**Does it match Fortran / APL / NumPy?**

| access | this proposal | Fortran | NumPy | APL |
|--------|---------------|---------|-------|-----|
| `A[i,j]` (full) | scalar load | `A(i,j)` element ✓ | `A[i,j]` scalar ✓ | `i j⌷A` element ✓ |
| `A[i]` (partial) | load / not a slice | `A(i)` on 2-D = **error** | `A[i]` = **row** (implicit) | `i⌷A` = **row** |
| row slice | `A[i,..]` / `SLICE` | `A(i,:)` explicit ✓ | `A[i,:]` (and `A[i]`) | `i⌷A` |

So it **matches Fortran** (full subscripts = element; explicit section = slice; *no*
implicit short-subscript rank-reduce — our `..` plays Fortran's `:`). It matches the
**explicit** forms of NumPy/APL but **drops their implicit `A[i]`-as-row** sugar.

**Tension to weigh:** `gaussj_dv.sis` uses the OPPOSITE convention — its header says
`% A[i] — rank reduce`, and `idfamax(A[i], n)` relies on `A[i]` being the row. Adopting
load-by-default means rewriting those to `A[i,..]` / `SLICE(A,...)`, and is the inverse
of the always-rank-reduce invariant.

**The choice in one line:**
- **Always-rank-reduce (APL/NumPy; current gaussj_dv intent):** `A[i]` = slice; scalar
  = rank-0 (full index); index-count-vs-rank decides. Convenient, needs the rule, and the
  `[ ]` is uniform across ranks.
- **Load-by-default (Fortran; this proposal):** `A[i,j]` = scalar; slice = explicit
  `..` / `SLICE`. Unambiguous, no rank inference, matches Fortran exactly — but no
  `A[i]`-as-row sugar, and `[ ]` means "load" while slicing needs a second syntax.
